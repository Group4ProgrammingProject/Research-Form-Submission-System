...
def upload(request):
	if request.POST:
		...
		if form.is_valid():
			a=form.save()
			messages.add_message(request, messages.SUCCESS, "The file was uploaded")
...
def list_files(request, subm_id):
    ...
    try:
    	submission = Submission.objects.get(id=subm_id)
    except:
    ...
    if request.method == 'POST':
        ...
        if all((x in request.FILES for x in ['app_form', 'app_info', 'con_form'])):
            ...
            return HttpResponse(template.render(context) % subm_id)
    else:
